...
func GetRawHistory(
	...
	if err = VerifyHistoryIsComplete(
		...
		log.ErrorWithCode(logger, errorcode.HistAPIGetHistoryUtilityOperationFailed, "getHistory: incomplete history", err,
	...
	if len(nextToken) == 0 && transientWorkflowTaskInfo != nil {
		if err := validateTransientWorkflowTaskEvents(nextEventID, transientWorkflowTaskInfo); err != nil {
			...
			metrics.ServiceErrIncompleteHistoryCounter.With(metricsHandler).Record(1)
			log.ErrorWithCode(logger, errorcode.HistAPIGetHistoryUtilityOperationFailed, "getHistory error", err,
...
func GetHistoryReverse(
	...
	case *serviceerror.DataLoss:
		// log event
		log.ErrorWithCode(logger, errorcode.HistAPIGetHistoryUtilityOperationFailed, "encountered data loss event", err, tag.WorkflowNamespaceID(namespaceID.String()), tag.WorkflowID(execution.GetWorkflowId()), tag.WorkflowRunID(execution.GetRunId()))
