...
func (r *HistoryImporterImpl) applyEvents(
	...
	if mutableStateSpec.IsBrandNew {
		if task.getFirstEvent().GetEventType() != enumspb.EVENT_TYPE_WORKFLOW_EXECUTION_STARTED {
			...
			log.ErrorWithCode(task.getLogger(), errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::applyEvents encountered mutable state vs events mismatch", err)
			return nil, false, err
		}
...
func (r *HistoryImporterImpl) applyStartEventsAndSerialize(
	...
	if newMutableState != nil {
		log.ErrorWithCode(task.getLogger(), errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::applyStartEventsAndSerialize encountered create workflow with continue as new case", err)
	}
...
func (r *HistoryImporterImpl) applyNonStartEventsAndSerialize(
	...
	if newMutableState != nil {
		log.ErrorWithCode(task.getLogger(), errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::applyNonStartEventsAndSerialize encountered create workflow with continue as new case", err)
	}
	token, err := r.persistHistoryAndSerializeMutableState(ctx, mutableState, mutableStateSpec)
	return token, err == nil, err
}
...
func (r *HistoryImporterImpl) commit(
	...
	if !mutableStateSpec.ExistsInDB {
		...
		if err := r.transactionMgr.CreateWorkflow(
			...
			log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit encountered error creating workflow", err)
			return err
		}
		return nil
	}
	...
	if err != nil {
		log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit unable to find workflow in DB", err)
		return err
	}
	defer func() {
	...
	if err != nil {
		log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit unable to find current version history from DB", err)
		return err
	}
	...
	if err != nil {
		log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit unable to find current version history from memory", err)
		return err
	}
	cmpResult, err := versionhistory.CompareVersionHistory(memCurrentVersionHistory, dbCurrentVersionHistory)
	if err != nil {
		log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit unable to compare current version history between mem vs DB", err)
		return err
	}
	if cmpResult == 0 {
	...
	if cmpResult < 0 {
		...
		if err != nil {
			log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit unable to update version history from DB", err)
			return err
		}
		if updated {
			err = serviceerror.NewInternal("current version history should not be updated")
			log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit unable to update version history when updated flag is true", err)
			return err
		}
		...
		if err := dbNDCWorkflow.GetContext().SetWorkflowExecution(ctx, r.shardContext); err != nil {
			log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit encountered error setting workflow execution", err)
		}
		return nil
	}
	...
	if err := r.transactionMgr.UpdateWorkflow(
		...
		log.ErrorWithCode(r.logger, errorcode.PersistNDCHistoryImporterOperationFailed, "HistoryImporter::commit encountered error updating workflow", err)
		return err
	}
	return nil
}
