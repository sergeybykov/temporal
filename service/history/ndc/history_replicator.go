...
func (r *HistoryReplicatorImpl) applyBackfillEventsWithNew(
	...
	if err := r.doApplyEvents(ctx, newTask); err != nil {
		log.ErrorWithCode(newTask.getLogger(), errorcode.HistoryHistoryNdcError, "nDCHistoryReplicator unable to create new workflow when applyBackfillEvents", err)
		return err
	}

	if err := r.doApplyBackfillEvents(ctx, task, r.applyBackfillEvents); err != nil {
		log.ErrorWithCode(newTask.getLogger(), errorcode.HistoryHistoryNdcError2, "nDCHistoryReplicator unable to create target workflow when applyBackfillEvents", err)
		return err
	}
	return nil
}
...
func (r *HistoryReplicatorImpl) applyStartEvents(
	...
	if newMutableState != nil {
		log.ErrorWithCode(task.getLogger(), errorcode.HistoryHistoryNdcError3, "HistoryReplicator::applyStartEvents encountered create workflow with continue as new case", err)
	}
	...
	if err != nil {
		log.ErrorWithCode(task.getLogger(), errorcode.HistoryHistoryNdcError4, "nDCHistoryReplicator unable to create workflow when applyStartEvents", err)
	} else {
...
func (r *HistoryReplicatorImpl) notify(
	...
	if clusterName == r.clusterMetadata.GetCurrentClusterName() {
		...
		log.WarnWithCode(r.logger, errorcode.HistoryReplicationTaskProcessorOperationFailed, "nDCHistoryReplicator applying events generated by current cluster")
		return
	}
