...
func (r *taskProcessorManagerImpl) handleClusterMetadataUpdate(
	...
	for clusterName := range newClusterMetadata {
		...
		if err != nil {
			log.ErrorWithCode(r.logger, errorcode.HistReplicationTaskProcessorOperationFailed, "Failed to get source shard id list", err, tag.ClusterName(clusterName))
			continue
		}
	...
	for {
		...
		case <-cleanupTimer.C:
			if err := r.cleanupReplicationTasks(); err != nil {
				log.ErrorWithCode(r.logger, errorcode.HistReplicationCleanupFailed, "Failed to clean up replication messages.", err)
...
func (r *taskProcessorManagerImpl) checkReplicationDLQSize() {
	...
	for clusterName := range r.shard.GetClusterMetadata().GetAllClusterInfo() {
		...
		if err != nil {
			log.ErrorWithCode(r.logger, errorcode.HistReplicationDLQOperationFailed, "Failed to check replication DLQ size.", err)
			return
		}
		if !isEmpty {
