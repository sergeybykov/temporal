...
func (e *taskExecutorImpl) Execute(
	...
	default:
		...
		log.ErrorWithCode(e.logger, errorcode.HistReplicationTaskExecutorOperationFailed, "Unknown replication task type.", nil, tag.ReplicationTask(replicationTask))
...
func (e *taskExecutorImpl) handleActivityTask(
	...
	case *serviceerrors.RetryReplication:
		...
		case nil:
			// no-op
		default:
			log.ErrorWithCode(e.logger, errorcode.HistReplicationTaskExecutorOperationFailed, "error resend history for history event", resendErr)
			return err
		}
		// This might be extra cost if the workflow belongs to local shard.
		// Add a wrapper of the history client to call history engine directly if it becomes an issue.
...
func (e *taskExecutorImpl) handleSyncWorkflowStateTask(
	...
	case *serviceerrors.RetryReplication:
		...
		case nil:
			...
			return err
		default:
			log.ErrorWithCode(e.logger, errorcode.HistReplicationTaskExecutorOperationFailed, "error resend history for replicate workflow state", resendErr)
			return err
		}
	default:
		return err
