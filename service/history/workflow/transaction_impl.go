...
func (t *TransactionImpl) CreateWorkflowExecution(
	...
	if err := NotifyNewHistorySnapshotEvent(engine, newWorkflowSnapshot); err != nil {
		log.ErrorWithCode(t.logger, errorcode.HistoryWorkflowTransactionOperationFailed, "unable to notify workflow creation", err)
	}
...
func (t *TransactionImpl) ConflictResolveWorkflowExecution(
	...
	if err := NotifyNewHistorySnapshotEvent(engine, resetWorkflowSnapshot); err != nil {
		log.ErrorWithCode(t.logger, errorcode.HistoryWorkflowTransactionOperationFailed, "unable to notify workflow reset", err)
	}
	if err := NotifyNewHistorySnapshotEvent(engine, newWorkflowSnapshot); err != nil {
		log.ErrorWithCode(t.logger, errorcode.HistoryWorkflowTransactionOperationFailed, "unable to notify workflow creation", err)
	}
	if err := NotifyNewHistoryMutationEvent(engine, currentWorkflowMutation); err != nil {
		log.ErrorWithCode(t.logger, errorcode.HistoryWorkflowTransactionOperationFailed, "unable to notify workflow mutation", err)
	}
