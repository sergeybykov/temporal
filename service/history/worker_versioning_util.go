...
func updateIndependentActivityBuildId(
	...
	defer func() {
		if retErr != nil {
			...
			taskTags := append(tasks.Tags(task), tag.Error(retErr))
			log.WarnWithCode(logger, errorcode.HistoryWorkflowExists, "failed to update activity's assigned build ID", taskTags...)
		}
		retErr = nil
	}()
