...
func (m *executionManagerImpl) readHistoryBranch(
	...
	for _, batch := range dataBlobs {
		...
		if len(events) == 0 {
			log.ErrorWithCode(m.logger, errorcode.PersistenceHistoryManagerOperationFailed, dataLossMsg, serviceerror.NewDataLoss(errEmptyEvents), dataLossTags(errEmptyEvents)...)
		...
		if firstEvent.GetVersion() != lastEvent.GetVersion() || firstEvent.GetEventId()+int64(eventCount-1) != lastEvent.GetEventId() {
			...
			log.ErrorWithCode(m.logger, errorcode.PersistenceHistoryManagerOperationFailed, dataLossMsg, serviceerror.NewDataLoss(errWrongVersion), dataLossTags(errWrongVersion)...)
		...
		if firstEvent.GetEventId() != token.LastEventID+1 {
			log.ErrorWithCode(m.logger, errorcode.PersistenceHistoryManagerOperationFailed, dataLossMsg, serviceerror.NewDataLoss(errNonContiguousEventID), dataLossTags(errNonContiguousEventID)...)
...
func (m *executionManagerImpl) filterHistoryNodesReverse(
	...
	for _, node := range nodes {
		...
		if lastTransactionID == 0 {
			log.WarnWithCode(m.logger, errorcode.CommonPersistenceMetricClientOperationFailed, "lastTransactionID is not set, this should not happen")
		}
