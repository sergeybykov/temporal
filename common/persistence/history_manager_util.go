...
func ValidateBatch(
	...
	if firstEvent.GetVersion() != lastEvent.GetVersion() || firstEvent.GetEventId()+int64(eventCount-1) != lastEvent.GetEventId() {
		...
		log.ErrorWithCode(logger, errorcode.PersistenceHistoryManagerOperationFailed, dataLossMsg, serviceerror.NewDataLoss(errWrongVersion), dataLossTags(errWrongVersion)...)
	...
	if lastEventID != 0 && firstEvent.GetEventId() != lastEventID+1 {
		log.ErrorWithCode(logger, errorcode.PersistenceHistoryManagerOperationFailed, dataLossMsg, serviceerror.NewDataLoss(errNonContiguousEventID), dataLossTags(errNonContiguousEventID)...)
